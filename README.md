...
- lane ごとに `include_current_time=true` を付けると、pre-call 注入に OpenClaw 風の `Current time` ブロックを追加する。これは正確な現在時刻として、相対日時の判断に使う
...
- dashboard 詳細画面の `idle minutes` 欄を、pre-call memory context の再注入タイミングとして編集する
  - lane ごとに独立して判定する。`0` の lane だけは毎 call、それ以外はその lane 自身の正の interval が経過した時だけ注入する
...
- `active_memory_directory` を設定すると、返答直前に現在の発言でその directory 配下の Markdown / text を軽量検索する
  ...
  - lane ごとに上位2件、各100文字（frontmatter込み）の短い抜粋だけを soft context として追加する
  - `read_active_memory(path=...)` は直近の active-memory retrieval で選ばれた path だけを、最大12,000文字で全文確認できる。任意の notes 読み取りには使わない
  - 設定の確認・編集と注入判定の確認は `memory_control(action=...)` を使う。通常の lane 編集は `memory_control(action="patch_lane", lane_name="all", changes={...})` で部分更新し、Active Memory の directory・有効/無効・selector・snapshot を他 lane に触れず変更する。`put_config` は whole-config 置換が必要な移行時だけ使う。
  - 関連候補が無い時は何も追加しない
