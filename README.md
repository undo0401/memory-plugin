...
- `reinject_interval_minutes` は pre-call memory context の再注入間隔として使う
  - `0` は毎回注入
  - `1` 以上は初回注入後、その分数が経過した会話でだけ再注入
- `idle_seconds` は background watcher 互換用に残すが、pre-call 注入の主判定は `reinject_interval_minutes` に寄せる
