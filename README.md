...
- plugin runtime path: `/opt/data/plugins/oc-memory/`
...
- auto deploy: 停止中
- runtime target: `/opt/data/plugins/oc-memory/`
...
- backend: `/opt/data/plugins/oc-memory/dashboard/plugin_api.py`
- config: `/opt/data/plugins/oc-memory/config/memory.json`
- runtime state: `/opt/data/plugins/oc-memory/state/memory-runtime.json`
- managed snapshots:
...
- dashboard 詳細画面の `idle_seconds` 欄が pre-call memory context の再注入タイミング正本
  - `0` は毎回注入
  - `1` 以上は初回注入後、その秒数（内部では分へ切り上げ換算）が経過した会話でだけ再注入
...
- `pre_context_command` を設定すると、差し込み対象になった時だけ返答前に任意 command を実行し、stdout を memory context に追加する
  - 空欄なら実行しない
  - `pre_context_timeout_seconds` は必須の安全弁で、未指定や不正値は 8 秒へ正規化する
  - command の stdin には lane / session / source / current_time metadata の JSON を渡す
  - stdout が JSON で `context` を持つ場合はそれを使い、それ以外は stdout 全体を markdown として扱う
  - timeout / non-zero exit は main response を止めず、runtime trace に残して skip する
